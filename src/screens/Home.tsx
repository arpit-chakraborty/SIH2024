import { StyleSheet, Text, View, SafeAreaView, Image, TouchableOpacity, ImageBackground, TextInput, FlatList, Modal } from 'react-native'
import React, { useContext, useState, useEffect, useRef } from 'react'
import Icon from 'react-native-vector-icons/FontAwesome';

import {launchCamera} from 'react-native-image-picker';
import AudioRecorderPlayer from 'react-native-audio-recorder-player';
import { addUserMessage, addBotMessage, Message, addMediaMessage } from '../components/ChatUtils';
...
const Home: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  const handleCameraPress = () => {
    launchCamera({mediaType: 'photo'}, (response) => {
      if (response.didCancel || response.errorMessage) return;  // Handle cancellation/error
      const imageUri = response.assets?.[0]?.uri;
      if (imageUri) {
        addMediaMessage(imageUri, setMessages, 'image');
        setTimeout(() => {
          addBotMessage(`Echo: Got Image`, setMessages);    // Simulate a bot response with a delay
        }, 500);
    ...
    if (!isRecording) {
      ...
      try {
        ...
        audioRecorderPlayer.addRecordBackListener((e) => {
          setRecordingTime(audioRecorderPlayer.mmss(Math.floor(e.currentPosition / 1000)));
          return;
        });
      } catch (error) {
      ...
      try {
        ...
        if (uri) {
          setAudioUri(uri);
          addMediaMessage(uri, setMessages, 'audio');
        }

        setTimeout(() => {
          addBotMessage(`Echo: Got Audio`, setMessages);    // Simulate a bot response with a delay
        }, 500);
        setInput('');
...
const handleSend = (input: string, setMessages: React.Dispatch<React.SetStateAction<Message[]>>) => {
  ...
  useEffect(() => {
    ...
    const renderItem = ({ item }: { item: Message }) => (
      <View style={[styles.messageBubble, item.sender === 'user' ? styles.userBubble : styles.botBubble]}>
        ...
        {item.type === 'audio' && (
            <TouchableOpacity onPress={() => audioRecorderPlayer.startPlayer(item.uri)}>
  ...
  return (
      ...
      <Modal visible={!!selectedImage} transparent={true} animationType="fade">
        <View style={styles.modalContainer}>
          <TouchableOpacity style={styles.closeButton} onPress={closeImageModal}>
            ...
            <Image source={selectedImage ? { uri: selectedImage } : require('../../assets/logo.png')} style={styles.fullImage} />
        </View>
      </Modal>
    ...
    <View style={styles.inputContainer}>
        ...
        />
        {!input && <>
          {isRecording ? <Text>{recordingTime}</Text>:
          ...
          <TouchableOpacity style={styles.iconButton}
            ...
            <Icon name={isRecording ? "microphone-slash" : "microphone"} size={25} color="black" />
          </TouchableOpacity>
        </>}
